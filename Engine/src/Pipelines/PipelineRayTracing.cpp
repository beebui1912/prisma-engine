...
Prisma::PipelineRayTracing::PipelineRayTracing(const unsigned int& width, const unsigned int& height) : m_width{width}, m_height{height} {
    ...
    m_hardwareMaxReflection = std::min(m_MaxRecursionDepth, contextData.device->GetAdapterInfo().RayTracing.MaxRecursionDepth);
    ...
    SamplerDesc SamLinearWrapDesc{
        FILTER_TYPE_LINEAR, FILTER_TYPE_LINEAR, FILTER_TYPE_LINEAR, TEXTURE_ADDRESS_WRAP, TEXTURE_ADDRESS_WRAP, TEXTURE_ADDRESS_WRAP  //
    };
    ...
    PipelineResourceDesc Resources[] = {
        ...
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::CONSTANT_LIGHT_SIZES.c_str(), 1, SHADER_RESOURCE_TYPE_CONSTANT_BUFFER, SHADER_RESOURCE_VARIABLE_TYPE_STATIC},
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::CONSTANT_DIR_DATA.c_str(), 1, SHADER_RESOURCE_TYPE_BUFFER_SRV, SHADER_RESOURCE_VARIABLE_TYPE_STATIC},
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_STATUS.c_str(), 1, SHADER_RESOURCE_TYPE_BUFFER_SRV, SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE},
        ...
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::CONSTANT_OMNI_DATA.c_str(), 1, SHADER_RESOURCE_TYPE_BUFFER_SRV, SHADER_RESOURCE_VARIABLE_TYPE_STATIC},
        ...
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_DIFFUSE_TEXTURE.c_str(), Define::MAX_MESHES, SHADER_RESOURCE_TYPE_TEXTURE_SRV, SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE, PIPELINE_RESOURCE_FLAG_RUNTIME_ARRAY},
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_NORMAL_TEXTURE.c_str(), Define::MAX_MESHES, SHADER_RESOURCE_TYPE_TEXTURE_SRV, SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE, PIPELINE_RESOURCE_FLAG_RUNTIME_ARRAY},
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_ROUGHNESS_METALNESS_TEXTURE.c_str(), Define::MAX_MESHES, SHADER_RESOURCE_TYPE_TEXTURE_SRV, SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE, PIPELINE_RESOURCE_FLAG_RUNTIME_ARRAY},
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::CONSTANT_LUT.c_str(), 1, SHADER_RESOURCE_TYPE_TEXTURE_SRV, SHADER_RESOURCE_VARIABLE_TYPE_STATIC},
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_PREFILTER.c_str(), 1, SHADER_RESOURCE_TYPE_TEXTURE_SRV, SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE},
        {SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_IRRADIANCE.c_str(), 1, SHADER_RESOURCE_TYPE_TEXTURE_SRV, SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE},
    ...
    m_pResourceSignature->GetStaticVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::CONSTANT_LIGHT_SIZES.c_str())->Set(LightHandler::getInstance().lightSizes());
    m_pResourceSignature->GetStaticVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::CONSTANT_DIR_DATA.c_str())->Set(LightHandler::getInstance().dirLights()->GetDefaultView(BUFFER_VIEW_SHADER_RESOURCE));
    m_pResourceSignature->GetStaticVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::CONSTANT_LUT.c_str())->Set(PipelineLUT::getInstance().lutTexture()->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE));
    m_pResourceSignature->GetStaticVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::CONSTANT_OMNI_DATA.c_str())->Set(LightHandler::getInstance().omniLights()->GetDefaultView(BUFFER_VIEW_SHADER_RESOURCE));
    ...
    m_updateData = [&]() {
        m_srb.Release();
        if (TLASHandler::getInstance().vertexData() && MeshIndirect::getInstance().statusBuffer() && PipelineSkybox::getInstance().isInit()) {
            ...
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, "g_TLAS")->Set(TLASHandler::getInstance().TLAS());
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, "vertexBlas")->Set(TLASHandler::getInstance().vertexData()->GetDefaultView(BUFFER_VIEW_SHADER_RESOURCE));
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, "primitiveBlas")->Set(TLASHandler::getInstance().primitiveData()->GetDefaultView(BUFFER_VIEW_SHADER_RESOURCE));
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, "locationBlas")->Set(TLASHandler::getInstance().vertexLocation()->GetDefaultView(BUFFER_VIEW_SHADER_RESOURCE));
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_DIFFUSE_TEXTURE.c_str())->SetArray(materials.diffuse.data(), 0, materials.diffuse.size(), SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_NORMAL_TEXTURE.c_str())->SetArray(materials.normal.data(), 0, materials.normal.size(), SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_ROUGHNESS_METALNESS_TEXTURE.c_str())->SetArray(materials.rm.data(), 0, materials.rm.size(), SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_STATUS.c_str())->Set(MeshIndirect::getInstance().statusBuffer()->GetDefaultView(BUFFER_VIEW_SHADER_RESOURCE));
            m_srb->GetVariableByName(SHADER_TYPE_RAY_MISS, "skybox")->Set(PipelineSkybox::getInstance().skybox()->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE));
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_PREFILTER.c_str())->Set(PipelinePrefilter::getInstance().prefilterTexture()->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE));
            m_srb->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, ShaderNames::MUTABLE_IRRADIANCE.c_str())->Set(PipelineDiffuseIrradiance::getInstance().irradianceTexture()->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE));
        }
    };

    TLASHandler::getInstance().addUpdates([&](auto vertex, auto primitive, auto index) { m_updateData(); });
...
void Prisma::PipelineRayTracing::render() {
    if (GlobalData::getInstance().currentGlobalScene()->meshes.size()) {
        ...
        MapHelper<RayTracingData> rayTracingData(contextData.immediateContext, m_raytracingData, MAP_WRITE, MAP_FLAG_DISCARD);
        ...
        contextData.immediateContext->CommitShaderResources(m_srb, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
...
RefCntAutoPtr<IPipelineState> Prisma::PipelineRayTracing::pso() { return m_pso; }

Prisma::PipelineRayTracing::~PipelineRayTracing() {}

RefCntAutoPtr<IShaderResourceBinding> Prisma::PipelineRayTracing::srb() { return m_srb; }

RefCntAutoPtr<ITexture> Prisma::PipelineRayTracing::colorBuffer() { return m_colorBuffer; }

void Prisma::PipelineRayTracing::maxRecursion(unsigned int recursion) { m_MaxRecursionDepth = recursion; }

Uint32 Prisma::PipelineRayTracing::maxRecursion() { return m_MaxRecursionDepth; }

void Prisma::PipelineRayTracing::maxRecursionReflection(unsigned int recursionReflection) { m_maxRecursionReflection = recursionReflection; }

Uint32 Prisma::PipelineRayTracing::maxRecursionReflection() { return m_maxRecursionReflection; }

Uint32 Prisma::PipelineRayTracing::hardwareMaxReflection() { return m_hardwareMaxReflection; }
