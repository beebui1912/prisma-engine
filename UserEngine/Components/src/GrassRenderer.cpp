...
void GrassRenderer::projection(glm::mat4 projection) { m_projection = projection; }
...
void GrassRenderer::generateGrassPositions(float density, float mult, float shift) {
    ...
    for (int i = 0; i < height * density; i++) {
        for (int j = 0; j < width * density; j++) {
            ...
            float interpolatedHeight = (1 - tx) * (1 - tz) * h00 + tx * (1 - tz) * h01 + (1 - tx) * tz * h10 + tx * tz * h11;
            ...
            glm::mat4 rotationMatrix = rotate(glm::mat4(1.0f), randomRotation, glm::vec3(0.0f, 1.0f, 0.0f)) * scaleMatrix;
