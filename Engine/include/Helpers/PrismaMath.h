...
constexpr auto oglToVkProjection = glm::mat4(1, 0, 0, 0, 0, -1, 0, 0,  // Flip Y
                                             0, 0, 1, 0, 0, 0, 0, 1);
...
static JPH::Vec3 JtoVec3(const glm::vec3& v) { return JPH::Vec3(v.x, v.y, v.z); }

// JPH::Vec3 to glm::vec3
static glm::vec3 JfromVec3(const JPH::Vec3& v) { return glm::vec3(v.GetX(), v.GetY(), v.GetZ()); }

// glm::vec4 to JPH::Vec4
static JPH::Vec4 JtoVec4(const glm::vec4& v) { return JPH::Vec4(v.x, v.y, v.z, v.w); }

// JPH::Vec4 to glm::vec4
static glm::vec4 JfromVec4(const JPH::Vec4& v) { return glm::vec4(v.GetX(), v.GetY(), v.GetZ(), v.GetW()); }
...
// glm::vec4 to JPH::Vec4
static JPH::Quat JtoQuat(const glm::quat& v) { return JPH::Quat(v.x, v.y, v.z, v.w); }

// glm::vec4 to JPH::Vec4
static glm::quat JtoQuat(const JPH::Quat& v) { return glm::quat(v.GetX(), v.GetY(), v.GetZ(), v.GetW()); }
...
static glm::vec3 getVec3(const aiVector3D& vec) { return glm::vec3(vec.x, vec.y, vec.z); }
...
static glm::quat getQuat(const aiQuaternion& pOrientation) { return glm::quat(pOrientation.w, pOrientation.x, pOrientation.y, pOrientation.z); }
