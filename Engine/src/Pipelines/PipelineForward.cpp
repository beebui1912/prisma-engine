...
Prisma::PipelineForward::PipelineForward(const unsigned int& width, const unsigned int& height) : m_width{width}, m_height{height} {
    createMSAA();
    create();
...
void Prisma::PipelineForward::render() {
    ...
    Prisma::ComponentsHandler::getInstance().updateTransparentRender(m_forwardTransparent->accum(), m_forwardTransparent->reveal(), PipelineHandler::getInstance().textureData().pDepthDSV);
...
void Prisma::PipelineForward::createAnimation()
{
    ...
    // Pipeline state name is used by the engine to report issues.
    // It is always a good idea to give objects descriptive names.
    PSOCreateInfo.PSODesc.Name = "Forward Animation Pipeline";
    ...
    RefCntAutoPtr<IShader> pVS;
    {
        ShaderCI.Desc.ShaderType = SHADER_TYPE_VERTEX;
        ShaderCI.EntryPoint = "main";
        ShaderCI.Desc.Name = "Forward Animation VS";
        ShaderCI.FilePath = "../../../Engine/Shaders/ForwardPipeline/vertex.glsl";
        contextData.device->CreateShader(ShaderCI, &pVS);
    }
    ...
    RefCntAutoPtr<IShader> pPS;
    {
        ShaderCI.Desc.ShaderType = SHADER_TYPE_PIXEL;
        ShaderCI.EntryPoint = "main";
        ShaderCI.Desc.Name = "Forward Animation PS";
        ShaderCI.FilePath = "../../../Engine/Shaders/ForwardPipeline/fragment.glsl";
        contextData.device->CreateShader(ShaderCI, &pPS);
    }
