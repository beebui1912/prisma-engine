...
void Prisma::GUI::VolumetricLight::volumetricSettings(VolumetricData blurData) { m_volumetricData = blurData; }

Prisma::GUI::VolumetricLight::VolumetricData Prisma::GUI::VolumetricLight::volumetricSettings() const { return m_volumetricData; }

void Prisma::GUI::VolumetricLight::createShaderVolumetric() {
    ...
    m_psoVolumetric->GetStaticVariableByName(Diligent::SHADER_TYPE_PIXEL, "depthTexture")->Set(PipelineHandler::getInstance().textureData().pDepthDSV->GetDefaultView(Diligent::TEXTURE_VIEW_SHADER_RESOURCE));
    m_psoVolumetric->GetStaticVariableByName(Diligent::SHADER_TYPE_PIXEL, "noiseTexture")->Set(m_textureNoise->GetDefaultView(Diligent::TEXTURE_VIEW_SHADER_RESOURCE));
    ...
    m_psoVolumetric->GetStaticVariableByName(Diligent::SHADER_TYPE_PIXEL, "FogSettings")->Set(m_volumetric);

    m_psoVolumetric->CreateShaderResourceBinding(&m_srbVolumetric, true);
    GlobalData::getInstance().addGlobalTexture({m_textureVolumetric, "Volumetric Blur"});
}
...
void Prisma::GUI::VolumetricLight::renderVolumetric() {
    auto& contextData = PrismaFunc::getInstance().contextData();
    ...
    blurData->densityMultiplier = m_volumetricData.densityMultiplier;
    blurData->fogColor = m_volumetricData.fogColor;
    blurData->maxDistance = m_volumetricData.maxDistance;
    blurData->stepSize = m_volumetricData.stepSize;
    blurData->densityMultiplier = m_volumetricData.densityMultiplier;
    blurData->noiseOffset = m_volumetricData.noiseOffset;

    blurData->densityThreshold = m_volumetricData.densityThreshold;
    blurData->noiseTiling = m_volumetricData.noiseTiling;

    blurData->lightContribution = m_volumetricData.lightContribution;
    blurData->lightScattering = m_volumetricData.lightScattering;
