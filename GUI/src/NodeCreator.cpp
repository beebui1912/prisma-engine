...
std::shared_ptr<Prisma::Node> Prisma::GUI::NodeCreator::createSpot() {
    auto parent = std::make_shared<Node>();
    parent->name("ParentSpot_" + std::to_string(parent->uuid()));
    auto light = std::make_shared<Light<LightType::LightSpot>>();

    parent->matrix(glm::rotate(glm::mat4(1), glm::radians(45.0f), glm::vec3(0.1, 1, 0.1)));

    light->name("Spot" + std::to_string(light->uuid()));
    parent->addChild(light);
    GlobalData::getInstance().currentGlobalScene()->root->addChild(parent);
    return light;
}
