...
Prisma::Animation::Animation(const std::string& animationPath, std::shared_ptr<AnimatedMesh> model) : m_animationPath{animationPath} {
...
std::shared_ptr<Prisma::Bone> Prisma::Animation::FindBone(const std::string& name) { return m_Bones[name]; }

int Prisma::Animation::ticksPerSecond() { return m_TicksPerSecond; }

float Prisma::Animation::duration() { return m_Duration; }

const Prisma::AssimpNodeData& Prisma::Animation::rootNode() { return m_RootNode; }

std::shared_ptr<std::map<std::string, Prisma::BoneInfo>> Prisma::Animation::boneIdMap() { return m_BoneInfoMap; }

std::string Prisma::Animation::name() const { return m_animationPath; }

std::string Prisma::Animation::path() const { return m_path; }

unsigned int Prisma::Animation::id() const { return m_id; }
...
void Prisma::Animation::ReadMissingBones(const aiAnimation* animation, std::shared_ptr<AnimatedMesh> model) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        m_Bones[name] = std::make_shared<Bone>(channel->mNodeName.data, boneInfoMap[channel->mNodeName.data].id, channel);
    }
